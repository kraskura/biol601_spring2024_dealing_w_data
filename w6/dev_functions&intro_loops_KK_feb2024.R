...
# What is the one, two, or a few variables that change, while everything else stays consistent?
...
mylist<-list(rainiscool=c(14,32,9,44),
             cofeeisdelicious=c(99,144,50,77))
...
ggformat<-function(plot, title="", y_title, x_title, print=TRUE, size_text = 15, color_text = "blue"){
  ...
  plot<- plot + # its like adding copy-paste lines to the ggplot
    ...
    ylab(y_title)+ 							
    xlab(x_title)+ 							
    theme(axis.text.y=element_text(size=size_text, colour= color_text),
...
# ggformat(plot = yeyplots, y_title = "cakfba", x_title = "sgdg", print = TRUE, size_text = 5)

ggformat(yeyplots,
         x_title = expression(Temperature~degree~C),
       y_title = expression(paste(Max~heart~rate~(f[Hmax]))), print = TRUE, size_text = 20)
...
for (i in 1:3) {
  ...
  # e.g. the ith round of following instructions written inside here
}
